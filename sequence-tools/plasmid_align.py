...
def main():
    ...
    missing = [f for f in args.fasta if not Path(f).exists()]
    if missing:
        sys.exit("ERROR: FASTA file(s) not found:\n" +
                 "\n".join(f"  {f}" for f in missing))

    print("=" * 60)
    print("  plasmid_align.py")
    print("=" * 60)
    print(f"  FASTQ:           {args.fastq}")
    print(f"  FASTA files:     {len(args.fasta)}")
    for f in args.fasta:
        print(f"                   {f}")
    print(f"  Min avg quality: Q{args.min_qual}")
    print(f"  Min length:      {args.min_length} bp")
    print(f"  Ambiguity delta: {args.ambiguity_delta}")
    print(f"  Circular mode:   {args.circular}")
    print(f"  Fill N regions:  {args.fill_n}")
    ...
    plasmids, orig_lens, n_cumsums = load_plasmid_library(
        args.fasta, circular=args.circular)
    if not plasmids:
        sys.exit("ERROR: no valid plasmid sequences loaded.")
    for name, seq in plasmids.items():
        effective_len = len(seq) // 2 if args.circular else len(seq)
        n_count = seq.upper().count("N")
        pct_n = 100 * n_count / len(seq)
        print(f"  {name:40s}  {effective_len:>8,} bp  "
    ...
    if args.fill_n:
        ...
        for name, seq in plasmids_orig_filled.items():
            ...
            plasmids[name] = (seq + seq) if args.circular else seq
        orig_lens = new_orig_lens
        n_cumsums = new_n_cumsums
        print("Building minimap2 indices from filled sequences …")
    else:
        print("\nBuilding minimap2 indices (map-ont preset) …")
    t0 = time.time()
    aligners = build_aligners(plasmids, args.threads, circular=args.circular)
    print(f"  {len(aligners)} indices built in {time.time()-t0:.1f}s")
    ...
    for name, seq, qual in iter_fastq(args.fastq):
        ...
        if n_passed % 500 == 0:
            ...
            print(f"    … {n_passed} reads aligned ({elapsed:.0f}s)", flush=True)

        hit_map = align_read(seq, aligners, circular=args.circular,
                             orig_lens=orig_lens, n_cumsums=n_cumsums)
        best, ambig, second, reason = classify_read(hit_map, args.ambiguity_delta)
    ...
    print(f"Per-plasmid summary → {args.summary}")
    ...
    print("  SUMMARY")
    print("=" * 60)
    print(f"  Total reads in FASTQ:       {n_total:>8,}")
    print(f"  Filtered (too short):       {n_short:>8,}")
    print(f"  Filtered (low quality):     {n_lowqual:>8,}")
    print(f"  Reads passed filters:       {n_passed:>8,}")
    print(f"  Unmapped:                   {n_unmapped:>8,}")
    print(f"  Ambiguous:                  {n_ambiguous:>8,}")
    print(f"  Uniquely assigned:          {n_passed - n_unmapped - n_ambiguous:>8,}")
    if args.circular and "is_junction_spanning" in results_df.columns:
        n_jspan = results_df["is_junction_spanning"].sum()
        print(f"  Junction-spanning:          {n_jspan:>8,}")
    print()
    print("  Per-plasmid assignments (unique):")
    for _, row in summary_df.iterrows():
        print(f"    {row['plasmid']:45s}  "
              f"unique={row['assigned_reads']:>5}  "
              f"ambig={row['ambiguous_reads']:>4}")
    print("=" * 60)
